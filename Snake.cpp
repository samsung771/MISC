...
class Game
{
	private:
		...
		void Setup()
		{
			...
			dir = STOP;
			score = 0;
			headPos = { (short)(dimensions.X / 2),(short)(dimensions.Y / 2) };
		...
		void updateScore()
		{
			createText(std::to_string(score), COORD{ (short)(dimensions.X+2 - (std::to_string(score).length()-1)/2),(short)(dimensions.Y + 2) }, WHITE);
		}
		...
		void updateSnake()
		{
			...
			switch (dir)
			{
				case UP: headPos.Y--; break;
				case LEFT: headPos.X--; break;
				case RIGHT: headPos.X++; break;
				case DOWN: headPos.Y++; break;
			}
